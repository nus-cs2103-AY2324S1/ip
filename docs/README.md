...
Make sure your system is running on Java 11, this might affect your experience while using the chat-bot.
...
> you encounter this error, try other accepted time formats.
> 
...
`help <COMMAND>` - provide a detailed description of the specified \<COMMAND\>, use this to learn more about the command and
it's proper syntax.
...
3. If there is a time only input, date will be set to next occurrence of the time.
...
#### Event tasks
Creates a time-sensitive task with both a specified start and end datetime.
...
| `event staycation /from 1 Jan, 1200 /to Jan 3, 12:00`  | <span style='color: green;'>Creates an event task for stay-cation for next occurrence of Jan 1 at 12:00 PM to Jan 3 12:00 PM for next occurence of the of date range</span> |
...
See [date time parsing errors](#date-time-parsing-errors) if using the given correct examples above give you errors.
...
Opens editing mode for the task specified by the index. Task specified will appear in the text field for user to modify. Modified task must follow the correct syntax of adding a task.

User can type `C` or `c` to exit editing mode for the task and resume normal mode.
...
> [!IMPORTANT]
> Task will only save upon `bye` command or `save` command. Please save your task list regularly to avoid losing it.
...
Oreo allows input of date only and time only commands when adding deadlines and events. Date inputs may or may not include a year. As such, there are certain underlying assumptions that Oreo will make based on the entered in date time. Understanding this would enable you to use the date time expansive date time format effectively.
...
```
This also helps reduce the redundancy for commands.
