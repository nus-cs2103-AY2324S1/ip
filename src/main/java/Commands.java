...
public class Commands {
    ...
    public static void deleteTasks(int taskNumber) throws  DukeException{
        if (count < taskNumber) throw new DukeException("Invalid input added");
    ...
    public static void handleInput(String userInput) throws InvalidTaskInput, DukeException{
            if (userInput.equalsIgnoreCase("bye")) return;
            if (userInput.equalsIgnoreCase("list")) {
                printList();
            } else if (userInput.length() > 7 && userInput.substring(0,6).equalsIgnoreCase("delete")) {
                // delete takes up first 5, 6 index takes up space, so 7 onwards should be numbers
