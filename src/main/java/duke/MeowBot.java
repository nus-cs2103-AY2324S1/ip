...
public class MeowBot extends Application {
    ...
    private void handleUserInput() {
        ...
        dukeText.setPadding(new Insets(5, 5, 5, 5));

        double radius = 50; // Adjust the radius as needed
        Circle userClip = new Circle(radius);
        userClip.setCenterX(radius);
        userClip.setCenterY(radius);

        Circle meowClip = new Circle(radius);
        meowClip.setCenterX(radius);
        meowClip.setCenterY(radius);

        ImageView userDp = new ImageView(user);
        ImageView meowBotDp = new ImageView(duke);

        userDp.setClip(userClip);
        meowBotDp.setClip(meowClip);

        dialogContainer.getChildren().addAll(
                DialogBox.getUserDialog(userText, userDp),
                DialogBox.getDukeDialog(dukeText, meowBotDp)
        );
