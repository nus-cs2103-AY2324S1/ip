...
public class Duke {
  ...
  private static final Map<String, CommandType> commands = Map.ofEntries(
      new SimpleEntry<>("list", CommandType.LIST),
      new SimpleEntry<>("mark", CommandType.MARK),
      new SimpleEntry<>("unmark", CommandType.UNMARK),
      new SimpleEntry<>("todo", CommandType.TODO),
      new SimpleEntry<>("deadline", CommandType.DEADLINE),
      new SimpleEntry<>("event", CommandType.EVENT),
      new SimpleEntry<>("remove", CommandType.REMOVE),
      new SimpleEntry<>("bye", CommandType.BYE)
    );
  private static final Map<CommandType, List<String>> arguments = Map.ofEntries(
      new SimpleEntry<>(CommandType.LIST, List.of()),
      new SimpleEntry<>(CommandType.MARK, List.of(defaultField)),
      new SimpleEntry<>(CommandType.UNMARK, List.of(defaultField)),
      new SimpleEntry<>(CommandType.TODO, List.of("description")),
      new SimpleEntry<>(CommandType.DEADLINE, List.of("description", "by")),
      new SimpleEntry<>(CommandType.EVENT, List.of("description", "from", "to")),
      new SimpleEntry<>(CommandType.REMOVE, List.of(defaultField)),
      new SimpleEntry<>(CommandType.BYE, List.of())
    );
  private static final Map<CommandType, Function<Map<String, String>, Task>> taskConstructors = Map.ofEntries(
      new SimpleEntry<>(CommandType.TODO, cmd -> new ToDo(cmd.get("description"))),
      new SimpleEntry<>(CommandType.DEADLINE, cmd -> new Deadline(cmd.get("description"), cmd.get("by"))),
      new SimpleEntry<>(CommandType.EVENT, cmd -> new Event(cmd.get("description"), cmd.get("from"), cmd.get("to")))
    );
  ...
  private static Map<String, String> parseInput(String[] arr) throws DukeException {
    ...
    Map<String, List<String>> disjointMap = new HashMap<>();
    ...
    Set<String> checkFields = new HashSet<>(expectedFields);
