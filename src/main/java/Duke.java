        ...
        while (sc.hasNext()) {
            ...
            try {
                if (input[0].equals("bye")) {
                    ...
                    if (taskIndex < 0 || taskIndex > tasks.size()) throw new DukeException("OOPS!!! Task does not exist.");
                    ...
                    if (input.length <= 1) throw new DukeException("OOPS!!! The description of a todo cannot be empty.");
                    String newTaskDesc = String.join(" ", input).substring(input[0].length() + 1);
                    newTask = new Todo(newTaskDesc);
                } else if (input[0].equals("deadline")) {
                    if (input.length <= 1) throw new DukeException("OOPS!!! The description of a deadline cannot be empty.");
                    ...
                    if (input.length <= 1) throw new DukeException("OOPS!!! The description of an event cannot be empty.");
                    String taskInput = String.join(" ", input).substring(input[0].length() + 1);
                    int fromIndex = taskInput.indexOf("/from");
                    int toIndex = taskInput.indexOf("/to");
                    if (fromIndex == -1 || toIndex == -1) throw new DukeException("OOPS!!! Please include the to and from date/time of the event.");
